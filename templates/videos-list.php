...
$aparat_thumbnail_base   = 'https://static.cdn.asset.aparat.com/avt/';
$aparat_player_base      = 'https://www.aparat.com/video/video/embed/videohash/';
...
foreach ( $videos_data as $video ){
	...
	elseif( $video['type'] == 'a' ){
		...
		if( ! empty( $aparat_videos[ $video_id ] )){
			$video_data          = $aparat_videos[ $video_id ];
			$video_data['thumb'] = $video_data['thumb'];
			$video_data['id']    = $aparat_player_base. $video_id .'/vt/frame';
			$video_data['iframe_class'] = "aparat-iframe";
		}
	}
	if( ! empty( $video_data ) ){
