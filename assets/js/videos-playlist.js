...
jQuery(document).ready(function(){
	...
	jQuery('.videos-block').each(function (idx, item){
		...
		$thisVideoBox.on( 'click', '.video-playlist-item:not(".is-playing")', function(){
			...
			// Aparat Custom
			let activeAparatFrame = jQuery(".video-playlist-item.aparat-iframe.active-aparat");	
			if(activeAparatFrame.length)
			{
				let activeAparatFrameId = activeAparatFrame.data("name");
				jQuery("#"+activeAparatFrameId).attr("src",jQuery("#"+activeAparatFrameId).attr("src"));
			}
			jQuery('.video-playlist-item').removeClass("active-aparat");
			if(jQuery(this).hasClass("aparat-iframe"))
			{
				jQuery(this).addClass("active-aparat");
			}
			// Aparat Custom
