...
if( ! class_exists( 'TIELABS_VIDEOS' ) ) {
	...
	class TIELABS_VIDEOS {
		...
		//Aparat
		static $aparat_api_base = 'http://www.aparat.com/etc/api/';																													
		...
		public static function get_video_info( $videos_list ){
			...
			$aparat_videos    = get_option( 'tie_aparat_videos' );																																																							  
			$youtube_updated = false;
			$vimeo_updated   = false;
			$aparat_updated   = false;							 
					...
					if( ! isset( $vimeo_videos[ $video_id ] ) ){
						...
						if( $video_info ){
							...
							$vimeo_updated = true;
								...
								// Aparat
				elseif( preg_match( "#(?<=aparat.com/v/)[^&\n]+#", $video, $matches )){
					...
					if( ! isset( $aparat_videos[ $video_id ] )){
						$video_data = self::get_aparat_info( $video_id );
						...
						if( $video_data ){
							...
							$aparat_videos[ $video_id ] = $video_data;
							$aparat_updated = true;
						}
					}
				}
			...
			if( $aparat_updated ){
				update_option( 'tie_aparat_videos', $aparat_videos );
			}			 
		...
		/*
		 * Get Aparat Video data
		 */
		private static function get_aparat_info( $vid ){

			// Build the Api request
			$api_url = self::$aparat_api_base.'video/videohash/'.$vid;
			...
			// Prepare the Video thumbnail
			if( ! empty( $Data['small_poster'] )){
				$video['thumb']  = $Data['small_poster'];
			}
